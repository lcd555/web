...
_timer.prototype = {
	tick : function() {
		...
		//采用分段标签形式
		if (_this.tag) {
			_this.tag['day'] && (_this.tag['day'].innerHTML = msObj.day);
			_this.tag['hour'] && (_this.tag['hour'].innerHTML = msObj.hour);
			_this.tag['mini'] && (_this.tag['mini'].innerHTML = msObj.mini);
			_this.tag['sec'] && (_this.tag['sec'].innerHTML = msObj.sec);
		} else {
			_this.element.innerHTML = _str
		}
